...
export async function subscribeToUpdates(
  ...
  if (existingContact != null) {
    return await updateExistingContact(existingContact.id, existingContact.email, firstName)
  }

  return await addNewContact(email, firstName)
}

async function updateExistingContact(id: string, email: string, firstName: string): Promise<State> {
...
function randomGreeting(name: string) {
  const greetings = [`Hello ${name}!`, `Hi ${name}!`, `Hey ${name}!`, `Welcome ${name}!`]
...
async function sendSubscriptionEmail(email: string, firstName: string) {
  ...
  const emailReactElement = (
    <WelcomeEmailTemplate unsubscribeUrl={unsubscribeUrl} firstName={firstName} />
  )
  ...
  return resend.emails.send({
    ...
    to: email,
    subject,
    html: await render(emailReactElement),
    text: await render(emailReactElement, {
      plainText: true,
    }),
    headers: {
